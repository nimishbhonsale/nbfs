...
pthread_mutex_t min_neighbor_lock;

void *client_t(void *threadid)
{
	int sockfd;
	int numbytes =0;
	int his_port;
	...
	their_addr.sin_port = htons(his_port);  // short, network byte order
	their_addr.sin_addr = *((struct in_addr *)he->h_addr);
	memset(their_addr.sin_zero, '\0', sizeof their_addr.sin_zero);
	char *host_ip;
	host_ip = inet_ntoa(*(struct in_addr*)he->h_addr);	// IP address of server
	...
	do	{
		...
		// checek if needs to exit
		// if -- 1. there is a need to shut down because the flag is true
		// OR    2. this is a REGULAR node, no need to retry
		if(shutdown_flag == true)
			pthread_exit(NULL);

		// once a regular node doesn't find its neighbor up,
		// it decreses the neighbor-count by one.
		// if that count reached below MinNeighbors count,
		// than as per the spec, we need to have a soft restart
		if(my_node_type == REGULAR)	{
			pthread_mutex_lock(&min_neighbor_lock);
			//printf("failed count: %d\n", failed_neighbors);
			failed_neighbors--;	// one more neighbor failed
			if(failed_neighbors < MinNeighbors)	{
				rejoin_flag = true;	// need to rejoin the network

				// set the 'failed_neighbors' to a large number
				// so that other threads will not execute 'kill_threads'
				// command and the node will shutdown gracefully
				failed_neighbors = 100*MinNeighbors;
				kill_threads();		// time to shutdown
			}
			pthread_mutex_unlock(&min_neighbor_lock);
			pthread_exit(NULL);
		}

		sleep(5);	// wait for 'Retry' time before attemting again
	}	while(1);
	...
	if((numbytes = recv(sockfd, buffer, Header.data_length, 0)) == -1)	{
		//perror("recv");
		kill_threads();
		pthread_exit(NULL);
	}
...
//	printf("client: HLLO Recd: port %d, name %s\n", hello_msg_data.host_port,
...
//			}
			...
			if(msg_parse(sockfd, Header, thread_num) == -1)	{
		...
		msg_forward(sockfd, fwd_event, thread_num);
